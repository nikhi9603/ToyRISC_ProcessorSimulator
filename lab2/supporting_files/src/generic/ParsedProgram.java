...
public class ParsedProgram {
	...
	{
		return code.get(programCounter - mainFunctionAddress);
	}
	...
	public static int parseDataSection(String assemblyProgramFile)
	{
		...
		// Store data staring from 1st memory location. 
		// At 0th location we denote the start of the code section.
		...
		{			
			...
			if(line.contains(".data"))										
			{
				...
				do
				{
					...
					{
						//TODO removed statements that were adding data to the code arraylist
						ParsedProgram.symtab.put(line.replaceAll("[^a-zA-Z]",""), address);
	...
	public static void parseCodeSection(String assemblyProgramFile, int firstCodeAddress)
	{
		...
		while(sc.hasNextLine())
		{
			...
			{
				ParsedProgram.code.add(address-mainFunctionAddress, getInstructionFromString(line, address));
				address++;
			}
		}
		sc.close();
